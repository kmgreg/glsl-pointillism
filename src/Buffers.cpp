...
void BufferSet::dumpBuffer( const char *which ) {
    ...
    cout << "  Sizes:  v " << vSize << " e " << eSize << " t "
         << tSize << " c " << cSize << " n " << nSize << endl;
}
...
///
/// The vertex buffer already contains all the attributes set by the
/// application program; this function binds it (and the associated
/// element array buffer), and then sets attribute pointers as indicated
/// by the parameter strings.
///
void BufferSet::selectBuffers( GLuint program,
    const char *vp, const char *vc, const char *vn, const char *vt ) {

    // sanity check - make sure the BufferSet has been set up
    if( !bufferInit ) {
        cerr << "ERROR - selectBuffers: buffer not initialized!" << endl;
        return;
    }
    ...
    //  The vertex buffer will contain the following sections,
    // in this order:
    //
    //    position (vSize bytes)
    //    color (cSize bytes)
    //    normals (nSize bytes)
    //    texture coordinates (tSize bytes)
    //
    // All of these are in the vertex buffer already, but we may not
    // be using them; we only set attribute pointers for the ones
    ...
    // do we also want the per-vertex color data?
    if( vc != NULL ) {
    ...
    // must always add the color size in, even if we're not using
    // color data, because it's already in the vertex buffer and
    // we need to skip over it to get to the next block of data.
    offset += cSize;
    ...
    // similarly, must alway add in the normal data size even if
    // we're not using this data
    offset += nSize;
    ...
    // sanity check!
    if( offset != (vSize + cSize + nSize + tSize) ) {
        cerr << "*** selectBuffers: size mismatch, offset " << offset
             << " parts " << vSize
             << " " << cSize << " " << nSize << " " << tSize << endl;
    }
}
